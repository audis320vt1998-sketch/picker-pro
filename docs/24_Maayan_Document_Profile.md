...
- The upload screen may call that one-image endpoint sequentially for up to
  ...
  opaque random document reference per logical page, not a filename. A closer
  replacement photo of the same page retains that reference; a different page
  requires a new batch. It rejects an unsupported, empty, or over-12 MB
  selected image before upload as a browser convenience only; the endpoint
...
- A reviewer may explicitly select a clearer replacement photo for a failed or
  unusable page. Selection removes only that page's prior draft/failure and
  row selections, then requires a separate explicit OCR action; other pages
  stay intact. The new image remains browser-held until that action.
